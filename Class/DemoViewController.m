...
- (void)viewDidLoad {
    ...
    for (NSDictionary *dic in arr) {
        LHYCardModel *model = [LHYCardModel new];
        [model setValuesForKeysWithDictionary:dic];
        [models addObject:model];
    }
    
    _cardSwitch = [[LHYCardSwitch alloc] initWithFrame:CGRectMake(0, 64, self.view.bounds.size.width, self.view.bounds.size.height - 64)];
